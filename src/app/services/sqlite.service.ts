...
export class SqliteService {
  ...
  async createGame(id:number,Name:string,date:string){
    let sql = 'INSERT INTO games (Id,Name,Date) VALUES(?,?,?)';
    const dbName = await this.getDbName();
    return CapacitorSQLite.executeSet({
      database: dbName,
      set: [
  ...
  async getAllGames() {
    ...
    return CapacitorSQLite.query({
      ...
      if (response.values) {
        for (let index = 0; index < response.values.length; index++) {
        const game = response.values[index];
        games.push({
          id: game.Id,
          name: game.Name,
          date: game.Date
  ...
  async getLastIdGame(){
    let sql = 'SELECT MAX(Id) as Id FROM games';
    const dbName = await this.getDbName();
    return CapacitorSQLite.query({
      database: dbName,
      statement: sql,
  ...
  async createShip(id:number,code: string, name: string, cost: number, hullSize: number) {
    // Sentencia para insertar un registro
    let sql = 'INSERT INTO ships (Id,Code, Name, Cost, HullSize) VALUES(?,?,?,?,?)';
    // Obtengo la base de datos
    const dbName = await this.getDbName();
    // Ejecutamos la sentencia
    return CapacitorSQLite.executeSet({
  ...
  async getAllShips() {
    ...
    return CapacitorSQLite.query({
      ...
      if (response.values) {
        for (let index = 0; index < response.values.length; index++) {
        const ship = response.values[index];
        ships.push({
          id: ship.Id,
          code: ship.Code,
          name: ship.Name,
